...
void drawFigures(Group* p){
    ...
    //for(o = p->ordem.begin();o != p->ordem.end();++o){
        aux = it->second;
        if(it->first == T){
    ...
    for (XMLElement* filho = grupo->FirstChildElement(); filho != NULL; filho = filho->NextSiblingElement()){
        ...
        { /** UMA TAG MODELS SO PODE TER UM MODEL **/
            XMLElement* neto = filho->FirstChildElement();
                ...
                if (ficheiro != NULL) {
                   // cout << ficheiro << endl;// APAGAR ISTO
...
int main(int argc, char **argv) {
    ...
    **/
    if (!loadOkay) {
        ...
        //XMLElement *modelo = scene->FirstChildElement("Group")->FirstChildElement("Models")->FirstChildElement("Model")->ToElement();

        //cout << modelo->Attribute("file") << endl;


        //string elemName = root->FirstChildElement()->Value();
        //cout << elemName << endl;
        c = new Scene();
    ...
    //glutCreateMenu(menu);
    //glutAddMenuEntry("Draw Vertices (Without Conection between Vertices)", 1);
    //glutAddMenuEntry("Draw Lines (Only the connection between Vertices)", 2);
    //glutAddMenuEntry("Fill triangles (Default)", 3);
    //glutAddMenuEntry("Keyboard camara",4);
    //glutAddMenuEntry("Mousse camara",5);
    //glutAddMenuEntry("Exit Program", 6);

    //glutAttachMenu(GLUT_RIGHT_BUTTON); //open menu with rightClick
